...
func toDatasetsUrlResponse(pid string, resp JobsResponse) (*api.DatasetsUrlResponse, error) {
	...
	result := api.DatasetsUrlResponse{
		Expires: time.Time{}, // use zero-time as sentinel for max
		Urls:    make([]api.UrlInfo, 0)}
...
func (s *DatasetsServiceImpl) logIn() (SciCatLoginResponse, error) {
...
func (s *DatasetsServiceImpl) isTokenExpired() bool {
	s.tokenMutex.RLock()
